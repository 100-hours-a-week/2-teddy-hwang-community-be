    ...
    email: (email) => {
        ...
        return {
            ...
            message: !email ? '이메일을 입력해주세요.' :
                    !isValid ? '올바른 이메일 형식이 아닙니다. (예:example@example.com)' : null
        }      
    },
    
    password: (password) => {
        const isValid = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#.~_-])[A-Za-z\d@$!%*?&#.~_-]{8,20}$/.test(password);
        return {
            isValid,
            message: !password ? '비밀번호를 입력해주세요.' :
                    !isValid ? '비밀번호는 8-20자의 대소문자, 숫자, 특수문자를 포함해야 합니다.' : null
        }      
    },
