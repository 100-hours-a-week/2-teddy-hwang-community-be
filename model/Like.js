...
const insertLike = async (postId, userId) => {
  return executeTransaction(async conn => {
    const [result] = await conn.query(LIKE_QUERIES.INSERT_LIKE, [
      userId,
      postId,
    ]);
...
const deleteLike = (postId, userId) => {
  return executeTransaction(async conn => {
    const [result] = await conn.query(LIKE_QUERIES.DELETE_LIKE, [
      userId,
      postId,
    ]);
    ...
    if (result.affectedRows === 0) {
      throw new BadRequest('좋아요 취소할 게 없습니다.');
    }
    return postId;
  });
};
...
const findPostLike = (postId, userId) => {
  return executeTransaction(async conn => {
    const [rows] = await conn.query(LIKE_QUERIES.FIND_POST_LIKE, [
      userId,
      postId,
    ]);
